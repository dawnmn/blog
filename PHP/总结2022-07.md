...
DOCKER
基于 Union FS 等技术，操作系统层面的虚拟化技术，Docker 不是虚拟机，容器就是进程。传统虚拟机技术是虚拟出一套硬件后，在其上运行一个完整操作系统，在该系统上再运行所需应用进程；而容器内的应用进程直接运行于宿主的内核，容器内没有自己的内核，而且也没有进行硬件虚拟。因此容器要比传统虚拟机更为轻便。
镜像：分层存储，镜像 不包含任何动态数据。相当于是一个 root 文件系统，docker pull，docker run，docker image ls，docker image rm，docker build
容器：镜像（Image）和容器（Container）的关系，就像是面向对象程序设计中的 类 和 实例 一样，镜像是静态的定义，容器是镜像运行时的实体。容器进程运行于属于自己的独立的 命名空间。docker containerId start
仓库：存储镜像的服务
DockerFile用于构建镜像，每个指令就是一层。镜像是多层存储，每一层的东西并不会在下一层被删除，会一直跟随着镜像。因此镜像构建时，一定要确保每一层只添加真正需要添加的东西，任何无关的东西都应该清理掉。
...
FROM nginx
RUN echo '\<h1>你好\</h1>' > /usr/share/nginx/html/index.html
COPY package.json /usr/src/app/		package.json是指定上下文的目录中的文件
ADD ubuntu-xenial-core-cloudimg-amd64-root.tar.gz /    不推荐
CMD 一个容器启动时要执行的命令，运行时可以指定新的命令来替代镜像设置中的这个默认命令。docker容器内没有后台服务的概念，容器中的应用都应该以前台执行。
ENTRYPOINT [ "curl", "-s", "http://myip.ipip.net" ] 当存在 ENTRYPOINT 后，CMD 的内容将会作为参数传给 ENTRYPOINT
ENV <key> <value> 设置环境变量
ARG 构建参数和 ENV 的效果一样，都是设置环境变量。所不同的是，ARG 所设置的构建环境的环境变量，在将来容器运行时是不会存在这些环境变量的。
VOLUME /data 容器运行时应该尽量保持容器存储层不发生写操作，对于数据库类需要保存动态数据的应用，其数据库文件应该保存于卷(volume)中。docker run -d -v mydata:/data xxxx
EXPOSE 仅仅是声明容器打算使用什么端口而已，并不会自动在宿主进行端口映射。
WORKDIR /app 如果需要改变以后各层的工作目录的位置（RUN命令执行的目录位置），那么应该使用 WORKDIR 指令。

docker build构建镜像，上下文：docker build命令或dockerfile文件所在目录为根目录，一般来说，应该会将 Dockerfile 置于一个空目录下，或者项目根目录下。如果该目录下没有所需文件，那么应该把所需文件复制一份过来。一般大家习惯性的会使用默认的文件名 Dockerfile。

多阶段构建

docker容器支持导入导出

.dockerignore文件的排除模式语法和 Git 的 .gitignore 文件相似。
.idea/
.git/
vendor/

Docker Composer，负责实现对 Docker 容器集群的快速编排。
...
BASE可靠事件队列：用户账号扣款、商家账号收款、库存商品例子，消息队列服务器将在每次轮询的时候持续地向未响应的服务重复发送消息，所有被消息服务器发送的消息都必须具备幂等性，通常的设计是让消息带上一个唯一的事务 ID，以保证一个事务中的出库、收款动作会且只会被处理一次。最大努力交付，不支持回滚。


TCC事务：Try-Confirm-Cancel
Try：尝试执行阶段，完成所有业务可执行性的检查（保障一致性），冻结相关资源，并且预留好全部需用到的业务资源（保障隔离性）。
...
整数 浮点数 布尔 字符串 数组 切片 map 指针 chan 函数 结构体 接口

没有隐式的数值转换，没有构造函数和析构函数，没有默认参数，没有继承

bufio os ioutil http flag strings bytes strconv
...
panic会立刻停止执行当前函数的剩余代码，触发当前Goroutine的defer
跟在defer后面的函数会在defer所在函数执行结束之后执行。可以在一个函数中执行多条defer语句，它们的执行顺序与声明顺序相反。关闭连接、释放锁、捕获panic等
Recover捕获异常作用：能够在发生panic时进行特殊处理

channels两个协程之间通信的机制
Channel还支持close操作，用于关闭channel，随后对基于该channel的任何发送操作都将导致panic异 常。对一个已经被close过的channel之行接收操作依然可以接受到之前已经成功发送的数据；如果 channel中已经没有数据的话会产生一个零值的数据。
...
无缓存 Channels有时候也被称为同步Channels。
向缓存Channel的发送操作就是向内部缓存队列的尾部插入元素，接收操作则是从队列的头部删除元素。如果内部缓存队列是满的，那么发送操作将阻塞直到因另一个goroutine执行接收操作而释放了新的队列空间。相反，如果channel是空的，接收操作将阻塞直到有另一个goroutine执行发送操作而向队列插入元素。
多个goroutines并发地向同一个channel发送数据，或从同一个channel接收数据都是常见的用法。
goroutines泄漏。
基于select的多路复用。
如何退出协程：1 select配合一个专门的管道，关闭管道退出 2 超时退出


并发访问变量：
1 用一个缓冲区为1的channel来保证最多只有一个goroutine在同一时刻访问一个共享变量。
2 sync.Mutex互斥锁 sync.RWMutex读写锁

Go内存同步：编译器和处理器常常会对指令做重排序/多核cpu机器，如果两个协程在不同的cpu核上执行，每个核有自己的缓存，这样一个协程的写入对于其他协程的Print，在主存同步之前就是不可见的
解决方式：使用锁或者channel
Go协程同步方式：channel sync.lock sync.WaitGroup(一个 WaitGroup对象可以等待一组协程结束)

sync.Once：针对初始化延迟的并发问题
var loadIconsOnce sync.Once
var icons map[string]image.Image
func Icon(name string) image.Image { loadIconsOnce.Do(loadIcons) return icons[name] }

gin-admin: 先初始化yaml配置文件 日志 redis mysql 路由（通过反射实现url对方法的绑定）session

context用来解决goroutine之间同步取消信号、传递值的功能。
