...
// 输出 3 4 2 1
在压栈function1的时候，需要连同函数地址、函数形参一同进栈
...
Goroutine 有简洁的模型：在相同的地址空间和其他 goroutine 并发地执行函数。
协程中避免共享变量，如err、指针、slice等。
...
占用内存更小（几 kb）
调度更灵活 (runtime 调度)
goroutine的创建理论上来说只受内存限制。
G 协程
M 线程
P 处理器
在程序执行的任意时刻都只有runtime.GOMAXPROCS()个 goroutine 在同时运行。
GMP 模型:
全局G队列
P的本地G队列
P列表
