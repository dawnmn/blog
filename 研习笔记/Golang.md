...
`切片内存泄漏`：当只是用一个大切片的一小段时，应当新建一个切片，append需要的那一小段，让大切片能够被gc回收。
...
go格式化输出动词：%v %+v(打印结构体时，会添加字段名) %#v(相应值的类型的Go语法表示) %t %d %x %o %b %f %c(输出整数对应的Unicode字符) %s %q %T %x %X %012.1f
...
**Channel** 是协程之间的通信机制，先入先出FIFO的有锁管道。内部数据结构是一个`循环队列`
```
type hchan struct {
   qcount   uint           // 循环队列元素个数
   dataqsiz uint           // 循环队列的大小
   buf      unsafe.Pointer // 循环队列的内存指针
