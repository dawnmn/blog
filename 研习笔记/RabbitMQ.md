...
连接：AMQP连接通常是长连接。
**连接与通道**：AMQP 0-9-1提供了通道（channels）来处理多连接，可以把通道理解成共享一个TCP连接的多个轻量化连接。一个特定通道上的通讯与其他通道上的通讯是完全隔离的。不要反复创建和删除连接和通道，一个线程应当复用同一个连接下的通道，不同的线程不能共享通道。应当控制连接数目，消费者和生产者使用不同的连接。对于go语言，应对普通业务，创建一个连接、2个通道即可。
