...
Redis分布式锁：加锁`SET resource-name token-string NX EX max-lock-time` 解锁 键过期或del
...
故障检测：占据多数的Cluster主节点都认为某个主节点挂起后，会将该节点标记为下线，并向所有节点广播下线消息。
故障转移：从节点收到下线消息，向所有主节点广播拉票消息。主节点用Raft 算法选主，新主会撤销旧主的slot指派，并将这些slot指派给自己，并广播通知集群。
